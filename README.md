...
for (const key in listOfBooks){
         if (listOfBooks[key].alreadyRead){
       console.log("The " + listOfBooks[key].title + " By " +
  listOfBooks[key].author + " . ");
  } else {
    console.log("The " + listOfBooks[key].title + " By " +
  listOfBooks[key].author + " . need to read it.");
  }
...
const movies = {
  title: "the terminal",
  duration:  60 ,
  stars: ["Tom hancks", "nobady" , "anybody" ]
}
for ( const key in movies){
  console.log(movies.title +", last for " + movies.duration + " mints. stars: " + movies.stars.join(', ') )
}
...
const str = "This is a this";
const lowerCaseParagraph = str.toLowerCase();
